...
export async function fetchRawCpmmPoolState(
  powfi,
  tokenA,
  tokenB,
  listedTokens = undefined
  ...
  const immTypes = raw.immFields.map((field) => field.type);
  const mutTypes = raw.mutFields.map((field) => field.type);

  const expectedImmTypes = [
    "ByteVec",
    "ByteVec",
    "ByteVec",
    "ByteVec",
  ...
  const expectedMutTypes = Array(8).fill("U256");

  if (
    immTypes.join(",") !== expectedImmTypes.join(",") ||
    mutTypes.join(",") !== expectedMutTypes.join(",")
  ) {
    throw new Error(
      "Unexpected RC38 TokenPair field types: " + poolId
    );
  }
  ...
  const token0Info = await resolveTokenById(
    powfi,
    rawToken0Id,
    listedTokens
  );
  const token1Info = await resolveTokenById(
    powfi,
    rawToken1Id,
    listedTokens
  );
