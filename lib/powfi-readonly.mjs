...
export async function discoverCpmmTokens(powfi, pairs = undefined) {
  ...
  for (const id of tokenIds) {
    tokens.push(
      await resolveTokenById(powfi, id, listedTokens)
    );
  }
...
export async function resolveToken(
  ...
  for (const token of [...listedTokens, ...cpmmTokens]) {
...
export async function resolvePool(powfi, tokenAInput, tokenBInput) {
  ...
  const tokenA = await resolveToken(powfi, tokenAInput, discoveredTokens);
  const tokenB = await resolveToken(powfi, tokenBInput, discoveredTokens);
